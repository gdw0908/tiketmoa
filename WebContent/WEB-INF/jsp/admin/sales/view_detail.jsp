      ...
      <div id="dialog" title="부품검색">
          <table class="style_3" id="item_list">
            ...
            <tr>
	          ...
	          <td><!-- ${item.paytyp == 'iche' ? '2.75 %' : item.paytyp == 'virtual' ? '440 원' : '3.85 %'} -->
	          <fmt:formatNumber type="number" pattern="###.##" value="${(item.m_commission/item.money)*100 }" /> %
	          <%-- <!--배송비를 뺀 금액의 수수료율 --> <fmt:formatNumber type="number" pattern="###.##" value="${(item.m_commission/item.totmoney)*100 }" /> % --%></td>
