...
<div id="main" data-ng-init="board.go(1)" data-ng-cloak>
  <div class="titlebar">
    <h2>관리자 게시판</h2>
    <div> <span>통합관리</span> &gt; <span class="bar_tx">회원사 공지 게시판</span> </div>
  </div>
  <div class="container"> 
    <div class="contents">
    <form name="searchFrm" method="post" data-ng-submit="board.go(1)">
      ...
      <table class="style_3" id="boardList">
        <colgroup>
          <c:if test="${sessionScope.member.group_seq eq '1' }"><col width="5%" /></c:if>
          <col width="5%" />
          <col width="*" />
          <col width="10%" />
          <col width="8%" />
        ...
        <tr>
          <c:if test="${sessionScope.member.group_seq eq '1' }"><th>선택</th></c:if>
          <th>번호</th>
          <th>제목</th>
          <th>작성일</th>
          <th>작성자</th>
        </tr>
        <c:if test="${sessionScope.member.group_seq eq '1' }"><tr data-ng-if="board.list.length==0"><td colspan="5">결과가 없습니다.</td></tr></c:if>
        <c:if test="${sessionScope.member.group_seq ne '1' }"><tr data-ng-if="board.list.length==0"><td colspan="4">결과가 없습니다.</td></tr></c:if>
        <tr data-ng-repeat="item in board.list">
          <c:if test="${sessionScope.member.group_seq eq '1' }"><td><input type="checkbox" value="{{item.article_seq}}"/></td></c:if>
