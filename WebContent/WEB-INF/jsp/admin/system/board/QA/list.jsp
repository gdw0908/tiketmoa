...
<div id="main" data-ng-init="board.go(1)" data-ng-cloak>
  <div class="titlebar">
    <h2>자주묻는질문</h2>
    <div> <span>통합관리</span> &gt; <span>시스템관리</span> &gt;<span class="bar_tx">자주묻는질문</span> </div>
  </div>
  <div class="container"> 
    <div class="tab_menu">
      <ul class="tab">
      	<li data-ng-class="{on:param.cate_seq=='0'}"><a data-ng-href="#/list/1/0">전체</a></li>
      	<li data-ng-repeat="item in group.list" data-ng-class="{on:param.cate_seq=='{{item.cate_seq}}'}"><a data-ng-href="#/list/{{item.group_code}}/{{item.cate_seq}}">{{item.subject}}</a></li>
      </ul>
    </div>
    <div class="contents">
    <form name="searchFrm" method="post" data-ng-submit="board.go(1)">
      ...
      <pagination total-page="board.totalPage" current-page="board.currentPage" on-select-page="board.go(page)"></pagination>
       <div class="btn_bottom">
        <div class="r_btn">
          <span class="bt_all"><span><input type="button" value="등록" class="btall" data-my-href="/write/{{param.group_seq}}"/></span></span>
          <a href="#" id="dialog-link"><span class="bt_all"><span><input type="button" value="카테고리등록"  class="btall" data-ng-click="categoryWrite()"/></span></span></a>
          <a href="#" id="dialog-link"><span class="bt_all"><span><input type="button" value="카테고리수정"  class="btall" data-ng-click="categoryModify()"/></span></span></a>
