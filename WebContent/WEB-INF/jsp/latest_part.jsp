                      ...
                      <ul>
                        ...
                        <div class="md_list brand_list">
                          <ul>
                            ...
                            <li>
                            <div class="md_top">
                              <p class="mt_img"><a href="/goods/view.do?menu=menu${fn:substring(item.part1, 8, 9) }&seq=${item.item_seq }"><img src="${item.thumb }" alt=""></a></p>
                              <p class="mt_btn"><a href="/goods/view.do?menu=menu${fn:substring(item.part1, 8, 9) }&seq=${item.item_seq }" target="_blank"><img src="/images/container/md_img_btn1.gif" alt="새창"></a><a href="#" onclick="return addCart('${item.item_seq }')"><img src="/images/container/md_img_btn2.gif" alt="장바구니"></a><c:choose><c:when test="${item.inquiry_yn eq 'Y' }"><a href="javascript:inquery_y();"><img src="/images/container/md_img_btn3.gif" alt="바로구매"></a></c:when><c:otherwise><a href="#" onclick="return directOrder('${item.item_seq }')"><img src="/images/container/md_img_btn3.gif" alt="바로구매"></a></c:otherwise></c:choose></p>
                            </div>
                            <div class="md_bottom">
                              <p class="mb_1"><strong>차량명</strong> : ${item.carmodelnm } ${item.cargradenm }</p>
                              <p class="mb_1"><strong>부품명</strong> : ${item.part3_nm }</p>
                              <p class="mb_2">
                              	<span class="mb_2_1"><b class="c1">${item.grade }</b>등급</span>
                              	<c:if test="${item.discount_rate > 0}">
                              	<span class="mb_2_2">${item.discount_rate }%↓</span>
                              	</c:if>
                              	<c:if test="${item.discount_rate > 0}">
                              	<span class="mb_2_3">${suf:getThousand(item.user_price) } 원</span>
                              	</c:if>
                              </p>
