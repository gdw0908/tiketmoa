  ...
  <div class="sub_wrap">
    ...
    <div class="address_pop">
      ...
      <div class="sub_list sub_list_a1">
        ...
        <ul>
          <li>
            <div> <span class="img_box"> <span class="img_b1"><a href="#"><img src="${item.thumb }" alt=""></a></span> </span> <span class="info"> <span class="order_num"><strong>주문번호 : ${item.orderno }</strong></span> <span class="in_t1"> <span class="first"><strong>${item.cargradenm } (${item.caryyyy })</strong></span> <span><strong>${item.productnm}</strong></span> <span><strong>${item.grade }등급</strong></span> </span> <span><strong>${item.com_nm}</strong> / ${item.sigungu_nm }</span> <span><strong>주문상태 : <b class="c1">${item.status_nm }</b></strong></span> <span><strong>합계 : <b class="c2">${suf:getThousand(item.amt) }</b> 원</strong></span> </span> </div>
          </li>
        </ul>
      </div>
      <div class="btn_bottom"><a href="/mobile/mypage/shopping/state/index.do?mode=m_refunds_form&cart_no=${item.cart_no }"><img src="/images/sub_2/btn_cancel_d1.gif" alt="환불신청"></a></div>
    </div>
  </div>
</div>
