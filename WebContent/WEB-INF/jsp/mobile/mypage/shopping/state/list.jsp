  ...
  <div class="sm_wrap">
      <div class="sm_top"> <span class="select_box s_menu_type">
        <select id="go_url_value" name="go_url_value" class="select_sm">
          ...
          <option value = "m_list3.state" <c:if test = "${params.mode eq 'm_list3'}">selected</c:if>>환불/입금내역</option>        </select>
    ...
    <div class="sub_wrap">
      <div class="sub_2_line line_2">
        <h3><img src="/images/mobile/sub_2/sub_2_title_5_2.gif" alt="주문/배송조회"></h3>
      </div>
      <div class="p_o_line">
        <div class="period_line">
          <form id="searchFrm" name="searchFrm" action="/mobile/mypage/shopping/state/list.do?mode=m_list1" method="post">
          <div class="sub_tab">
            <ul class="tab">
      ...
      <c:choose>
      	...
      	<c:otherwise>
      		<c:forEach var="item" items="${data.list }" varStatus="status">
		        <div class="sub_list sub_list_a1" orderno="${item.orderno }">
		          ...
		          <ul>
		            <li>
		              <div> 
			              <span class="img_box"> 
			              	<span class="img_b1">
			              		<c:if test="${item.status eq '7' || item.status eq '8' }"><a href="javascript:track('${item.cart_no}');"></c:if>
				                <img src="${item.thumb }" alt="${item.productnm }">
				                <c:if test="${item.status eq '7' || item.status eq '8' }"></a></c:if>
			              	</span> 
			              ...
			              <span class="info"> 
			              	...
			              	<span class="in_t1"> 
			              		<span class="first">
			              			<strong>${item.cargradenm } (${item.caryyyy })</strong>
			              		</span> 
			              		<span>
			              	 ...
			              	 <span>
			              	 	<strong>${item.com_nm}</strong> / ${item.sigungu_nm }
			              	 </span>
			              	 <span>
			              	 	<strong>주문일 :</strong>${dtf:simpleDateFormat( item.reg_dt, 'yyyy-MM-dd HH:mm:ss' , 'yyyy-MM-dd') }
			              	 </span>
			          	 		<span>
			          	 			<strong>주문상태 : <b class="c1">${item.status_nm }</b></strong>
			          	 		</span>
		              	 		<span>
		              	 			<strong>주문수량 : ${item.qty } 개</strong>
		              	 		</span>
		              	 		<span>
		              	 			<strong>합계 : <b class="c2">${suf:getThousand(item.amt) }</b> 원</strong>
		              	 		</span>
