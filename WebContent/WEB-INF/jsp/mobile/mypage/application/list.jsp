...
<div class="sm_wrap">
  <div class="sm_top"> <span class="select_box s_menu_type">
    <select id="url" name="url" class="select_sm">
      ...
      <option value="/mobile/mypage/application/index.do?menu=menu1">부품문의 국산차</option>
      <option value="/mobile/mypage/application_import/index.do?menu=menu1">부품문의 수입차</option>
    </select>
    </span> <span class="sm_btn"><a href="javascript:;"><img src="/images/mobile/sub_2/sub_menu_a.gif" alt="메뉴이동" onclick = "go_url();"></a></span> </div>
</div>
<div class="sub_wrap">
  <div class="sub_line">
    <h3><img src="/images/mobile/sub_2/sub_2_title_8.gif" alt="부품문의 국산차"></h3>
  ...
  <table class="list_style_1">
    ...
    <tbody>
      <c:if test="${empty list }">
        <tr>
          <td colspan="3">등록 된 게시물이 없습니다.</td>
        </tr>
      </c:if>
      <c:forEach items="${list }" var="article" varStatus = "status">
        <tr>
          <td>${article.groupcodenm }</td>
          <td class="title"><a href="${servletPath }?mode=view&amp;article_seq=${article.article_seq }&amp;cpage=${params.cpage }&amp;rows=${params.rows }&amp;condition=${params.condition }&amp;keyword=${params.keyword }&amp;menu=menu1"> ${article.title }${article.public_yn == 'Y' ? '' :' (비공개)'}
  ...
  <div class="vr_btn"><a href="${servletPath }?menu=menu1&mode=insertForm"><img src="/images/article/board_btn6.gif" alt="문의등록"></a></div>
  ...
  <div class="bottom_search">
    <form action="${servletPath }" name="articleSearchForm" id="articleSearchForm" method="post" onsubmit="return goPage(1);">
      <input type="hidden" name="menu" value="menu1"/>
      ...
      </span> <span class="bottom_search_bt">
