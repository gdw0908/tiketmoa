...
<div class="sm_wrap">
  <div class="sm_top"> <span class="select_box s_menu_type">
    <select id="url" name="url" class="select_sm">
      ...
      <div class="sub_wrap">
        <div class="sub_line">
          <h3><img src="/images/mobile/sub_2/sub_2_title_2_1.gif" alt="파츠모아 이야기"></h3>
		...
		<div class="sub_list">
          ...
          <c:if test="${empty list }">
			<li style="text-align:center;">등록 된 게시물이 없습니다.</li>
		  </c:if>
          <c:forEach items="${list }" var="article" varStatus = "status">
				<li>
					<a href="${servletPath }?mode=view&amp;article_seq=${article.article_seq }&amp;cpage=${params.cpage }&amp;rows=${params.rows }&amp;condition=${params.condition }&amp;keyword=${params.keyword }&amp;menu=menu2">
						<span class="img"><img src="${article.file_thumb }" alt="<c:out value="${article.title }" escapeXml="true"/>"/></span>				
						<span class="info" style="padding-top:5px;">
							<strong>${article.title }</strong>
							<span style="margin-top:7px;">
								<strong>${article.reg_nm}</strong>
							</span>
							<span style="margin-top:7px;">
								<strong>[ ${article.reg_dt } ]</strong>
							</span>
						</span>
					</a>
				</li>
