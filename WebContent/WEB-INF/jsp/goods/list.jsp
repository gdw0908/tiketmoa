...
<body>
      ...
      <div class="sub_line">
        <div class="sl_wrap">
          <div class="sl_l" id="content_view">
          	<c:choose>
          		<c:when test="${param.menu eq 'menu1' }"><h3><img src="/images/sub/body_title.gif" alt="바디 부품정보"></h3></c:when>
          		<c:when test="${param.menu eq 'menu2' }"><h3><img src="/images/sub/title_2.gif" alt="의장 부품정보"></h3></c:when>
          		<c:when test="${param.menu eq 'menu3' }"><h3><img src="/images/sub/title_3.gif" alt="엔진 부품정보"></h3></c:when>
          		<c:when test="${param.menu eq 'menu4' }"><h3><img src="/images/sub/title_4.gif" alt="샤시 부품정보"></h3></c:when>
          		<c:when test="${param.menu eq 'menu5' }"><h3><img src="/images/sub/title_5.gif" alt="국산차 부품정보"></h3></c:when>
          		<c:when test="${param.menu eq 'menu6' }"><h3><img src="/images/sub/title_6.gif" alt="수입차 부품정보"></h3></c:when>
            ...
            <span>
           		<c:choose>
          			<c:when test="${param.menu eq 'menu1' }">바디</c:when>
          			<c:when test="${param.menu eq 'menu2' }">의장</c:when>
          			<c:when test="${param.menu eq 'menu3' }">엔진</c:when>
          			<c:when test="${param.menu eq 'menu4' }">샤시</c:when>
          			<c:when test="${param.menu eq 'menu5' }">국산차</c:when>
          			<c:when test="${param.menu eq 'menu6' }">수입차</c:when>
    ...
    <div class="sub_wrap">
      <div class="sub_contents">
			...
			<div class="sc_top">
		          	<ul class="sc_tab">
		            <li><a <c:if test = "${empty param.focus}">class="on"</c:if> href="javascript:go_url('${paramset }show=${param.show }&rows=${param.rows }');">신상품</a></li>
...
<!-- 		            <th>판매자</th> -->
		          ...
		          <tbody>
		          <c:if test="${empty data.list }"><tr><td colspan="4">조회 건수가 없습니다.</td></tr></c:if>
		          <c:forEach var="item" items="${data.list }">
		          <tr>
		            <td class="first">
		              <div class="sl_box">
		                <p class="sl_img"><a href="view.do?menu=${param.menu }&seq=${item.item_seq }"><img src="${item.thumb }" alt="${item.part3_nm }" style="width:172px;height:128px;"></a></p>
		                <p><a href="view.do?menu=${param.menu }&seq=${item.item_seq }" target="_blank"><img src="/images/container/md_img_btn1.gif" alt="새창"></a><a href="#" onclick="return addCart('${item.item_seq }')"><img src="/images/container/md_img_btn2.gif" alt="장바구니"></a><c:choose><c:when test="${item.inquiry_yn eq 'Y' }"><a href="javascript:inquery_y();"><img src="/images/container/md_img_btn3.gif" alt="바로구매"></a></c:when><c:otherwise><a href="#" onclick="return directOrder('${item.item_seq }')"><img src="/images/container/md_img_btn3.gif" alt="바로구매"></a></c:otherwise></c:choose>
		                </p>
		              </div>
		            </td>
		            ...
		            <td class="info">
			            <c:choose>
			            	<c:when test="${item.gubun eq '1' }">
				              <a href="view.do?menu=${param.menu }&seq=${item.item_seq }">
					          <span><strong>제품분류</strong> : <span class="c1">${item.part1_nm } / ${item.part2_nm }</span></span>
				              <span><strong>차량명</strong> : ${item.carmodelnm } (${item.caryyyy }년식)</span>
				              <span><strong>부품명</strong> : ${item.part3_nm }</span>
				              <span><strong>부품등급</strong> : ${item.grade }등급</span>
				              </a>
			            	</c:when>
			            	<c:when test="${item.gubun eq '2' }">
				              <a href="view.do?menu=${param.menu }&seq=${item.item_seq }">
					          <span><strong>제품분류</strong> : <span class="c1">수입차(부품용)</span></span>
				              <span><strong>차량명</strong> : ${item.carmodelnm } (${item.caryyyy }년식)</span>
				              <span><strong>부품등급</strong> : ${item.grade }등급</span>
				              </a>
			            	</c:when>
			            	<c:when test="${item.gubun eq '3' }">
				              <a href="view.do?menu=${param.menu }&seq=${item.item_seq }">
					          <span><strong>제품분류</strong> : <span class="c1">수출용/부품용(완차)</span></span>
				              <span><strong>차량명</strong> : ${item.carmodelnm } (${item.caryyyy }년식)</span>
				              <span><strong>부품등급</strong> : ${item.grade }등급</span>
				              </a>
			            	</c:when>
		            	...
		            	<c:choose>
				        	...
				        	<c:otherwise>
								...
								<c:if test="${item.discount_rate == 0 || empty item.discount_rate}">
					              <p class="price">${suf:getThousand(item.user_price) } 원<p>
				            	</c:if>
				        	</c:otherwise>
				        </c:choose>
		            </td>
		            <!-- 
		            <td class="btn_type">
		              <p><img src="/images/container/md_b_btn2.gif" alt="장착점"></p>
		            </td>
					 -->
		            <%-- <td class="lh_style_1">
		              <p><strong>${item.com_nm }</strong></p>
		              <p>${item.sigungu_nm }</p>
		
		              <p class="phone"><strong>${item.staff_tel }</strong></p>
		            </td> --%>
		          </tr>
		          </c:forEach>
		          </tbody>
