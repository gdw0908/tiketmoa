...
function status_popup(cart_no){
	$("#modal_dialog2").load("/popup/mypage/shopping/state/index.do?mode=status_popup", {cart_no : cart_no});
...
function cancelBank(f){
	...
	$("#modal_dialog").load("/popup/mypage/shopping/state/index.do?mode=refunds_send", {cart_no:f.cart_no.value, form_submit:f.form_submit.value, bank:f.bank.value, accountNo:f.accountNo.value});
	$("#modal_dialog").dialog("open");
}
...
<div id="sub">
        <div class="strapline">
          <h3><img src="/images/sub_2/h3_img_4_3.gif" alt="주문/반품/교환/조회"></h3>
          <div class="state"> <span>홈</span> &gt; <span>고객센터</span> &gt; <span><strong>나의쇼핑</strong></span> </div>
        </div>
        <div class="contents">

          <div class="period_line">
          <form id="searchFrm" name="searchFrm" action="index.do?mode=list2" method="post">
      	...
      	<c:otherwise>
      		...
      		<c:if test="${(item.status eq '2') or (item.status eq '3') or (item.status eq '4') or (item.status eq '5') or (item.status eq '19') or (item.status eq '10') or (item.status eq '12') or (item.status eq '20') or (item.status eq '21') or (item.status eq '22') }"><%-- 취소 종류일 경우 --%>
              ...
              <div class="product_box">
                ...
                <div class="pb_r_2">
                  ...
                  <p class="last">
                   ...
                   <c:if test="${status_view eq 'Y'}"><a href="javascript:status_popup('${item.cart_no }')"><img src="/images/sub_2/cart_btn2.gif" alt="상세내역"></a></c:if>
                  </p>
                </div>
              </div>
            </td>
...
<div id="modal_dialog2" title="취소신청">
</div>
</body>