...
<div id="sub">
        <div class="strapline">
          ...
          <div class="state">
            <span>홈</span> &gt; <span>고객센터</span> &gt; <span>카올바로</span> &gt; <span><strong>정비사례</strong></span>
          </div>
        </div>
        <div class="contents">
...
<div class="view_top">
  <p class="btn_box">
  <a href="carallbaro_list.do?type_state=${params.type_state }&amp;cpage=${params.cpage }&amp;rows=${params.rows }&amp;condition=${params.condition }&amp;keyword=${params.keyword }"><img src="/images/article/board_btn3.gif" alt="목록으로"></a>
  </p>
</div>
  ...
  <c:forEach items="${comment }" var="comment" varStatus = "status">
    <li<c:if test="${comment.lvl > 0 }"> class="comm_2"</c:if> id="comment${comment.comment_seq}">
	    <div class="comment_top">
	      <p class="ct_l"><strong<c:if test="${comment.lvl > 0 }"> class="comm_arrow"</c:if>>${comment.reg_nm != null ? comment.reg_nm : comment.reg_id }</strong> <span>${comment.reg_dt }</span></p>
	      <p class="ct_r">
	    ...
	    <%-- <c:if test="${sessionScope.member != null }">
	    <div class="area_box" id="commentReply${comment.comment_seq}" style="display:none;"><!-- 댓글 로그인 사용자만 -->
	      <form name="comment${comment.comment_seq }" action="/mypage/user_comment/index.do" method="post" onsubmit="return commentReply(this);">
	      <input type="hidden" name="reply_id" value="${comment.reg_id }"/>
	      <input type="hidden" name="comment_seq" value="${comment.comment_seq }"/>
	      <input type="hidden" name="mode" value="commentReply"/>
	      <p><strong class="comm_arrow">${sessionScope.member.member_id } @${comment.reg_id } 님께 댓글쓰기</strong> <a class="mod" href="#comment${comment.comment_seq}" onclick="commentReplyOnOff('${comment.comment_seq}','close')"><img src="/images/article/comm_btn_3.png" alt="취소"></a></p>
...
<form name="commentDelete" action="/mypage/user_comment/index.do" method="post" onsubmit="return commentDel(this);">
