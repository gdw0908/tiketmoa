...
<meta name="description" content="안녕하세요  db sound 입니다." />
<meta name="Keywords" content="db sound, 음향기기, 중고음향기기, 중고악기, 중고 쇼핑몰, 중고 악기 쇼핑몰, 중고 음향기기 쇼핑몰" />
<title>상품상세보기</title>
...
<!--         				<h3>소스/프로세서</h3> -->
<%--         			</c:when> --%>
<%--         			<c:when test="${param.menu == 'menu7'}"> --%>
<!--         				<h3>랙/하드케이스</h3> -->
<%--         			</c:when> --%>
<%--         			<c:when test="${param.menu == 'menu8'}"> --%>
<!--         				<h3>스탠드/보면대</h3> -->
<%--         			</c:when> --%>
<%--         			<c:when test="${param.menu == 'menu9'}"> --%>
<!--         				<h3>케이블/커넥터</h3> -->
<%--         			</c:when> --%>
<%--         			<c:when test="${param.menu == 'menu10'}"> --%>
<!--         				<h3>영상기기</h3> -->
<%--         			</c:when> --%>
<%--         			<c:when test="${param.menu == 'menu11'}"> --%>
<!--         				<h3>패키지</h3> -->
<%--         			</c:when> --%>
<%--         		</c:choose> --%>
<%--         	</c:when> --%>
...
<div class="sub_wrap">
  <div class="sub_contents" style="position: relative;">
  	<h3 class="order_line">홈 > 카테고리 > 상세 페이지</h3>
    <div class="details_1" >
      <div id="galleryWrap">
          ...
          <div class="thumbnails_body">
            <ul>
              <c:forEach var="item" items="${files }" varStatus="status">
                <li>
                  <div class="thumbitem" rel="${status.index }"  num="${status.index }" pg="${status.index }"> <img src="/upload/board/${item.yyyy }/${item.mm }/${item.uuid }" align="absmiddle" rel="${status.index }"  class='thumbimg'  style="width: 100%; height: 100%; object-fit: cover;"></div>
                </li>
              </c:forEach>
            </ul>
          </div>
      ...
      <div class="details_info" >
        <p class="details_title"><strong>${view.prod_nm } </strong>/ ${view.prod_classnm }</p>
        <table class="details_style_1">
          ...
          <th class="price">판매가격</th> 
              <td class="big_f">${suf:getThousand(view.prod_price) } <b>원</b>
              	<%-- <c:choose>
...
<%--               <a class="cart_btn" href="javascript:cart('${param.seq }')">장바구니</a> --%>
              <a class="cart_btn" href="/mypage/shopping/cart/index.do?mode=direct_order&seq=${param.seq }&qty=1">장바구니</a>
	  ...
	  <article class="content_container">
	  	<div class="wrap">
			...
			<section id="tab-1" class="tab-content current">
    			...
    			<div class="content"><img src="../images/sub/poduct_img.png"></div>
			</section>
			...
			<section id="tab-2" class="tab-content" >
				<div class="guide_content">
    				...
    				<ul>
    					...
    					<li>단, 부피가 큰 부품이나 고객이 원할 경우 1톤, 다마스 차량으로 운송하며, 지방의 경우 화물택배로 배송하여드립니다.</li>
    					<li>기타 궁금하신 사항은 1661-8431 고객센터로 문의주시면 언제든지 상세히 답변 드리도록 하겠습니다.</li>
				...
				<div class="guide_content">
    				...
    				<ul>
    					...
    					<li>- 택배 가능 부품(한사람이 들 수 있는 무게/부피가 작은 부품)은 기본이 5,000원이며, 무게/부피에 따라 배송비가 증가 됩니다.</li>
    					<li>- 택배 불가능한 부품은 경동택배를 통해 발송되며, 배송비는 기본 5,000원으로 무게/부피에 따라 배송비가 증가 됩니다.</li>
    				</ul>
				</div>
				<div class="guide_content">
	    			<h3>3. 교환 및 반품안내</h3>
    				<ul>
    					...
    					<li>단, 제품하자 시에는 1661-8431 고객센터로 문의 주시면 처리절차에 대해 친절하게 안내해 드리겠습니다.</li>
    				</ul>
				</div>
			</section>
	  	</div>
