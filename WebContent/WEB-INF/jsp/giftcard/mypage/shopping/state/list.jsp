	...
	<div id="sub">
		<div class="contents">
			...
			<div class="period_line">
				<form id="searchFrm" name="searchFrm" action="index.do?mode=list1"
					method="post">
									...
									<tr>
										...
										<td class="cart_main">
											<div class="product_box">
												<div class="pb_l">
													<a
														href="/giftcard/goods/view.do?menu=menu${fn:substring(item.part1, 8, 9) }&seq=${item.item_seq }">
												...
												<div class="pb_r_2">
													<p>
														<a
															href="/giftcard/goods/view.do?menu=menu${fn:substring(item.part1, 8, 9) }&seq=${item.item_seq }">
															<span><strong>${item.MAKERNM }</strong></span> <span><strong>${item.productnm }</strong></span>
														</a>
													</p>
													<p class="last">
										...
										<td>
											...
											<c:if test="${item.status eq '7' || item.status eq '8' }">
												<p class="btn_m2">
													<a href="#" onclick="track('${item.cart_no}')">배송추적</a>
												</p>
											</c:if>
										</td>
										<td class="b_none"><c:if test="${item.status eq '99' }">
												<p class="btn_m1">
													<a href="#" onclick="order_cancel('${item.cart_no}');">주문취소</a>
												</p>
											</c:if> <c:if test="${item.status eq '1' }">
												<p class="btn_m1">
													<a href="#" onclick="cancel_popup('${item.cart_no}');">취소신청</a>
												</p>
											</c:if> <c:if test="${item.status eq '8' || item.status eq '18'}">
												<p class="btn_m1">
													<a href="#">수취확인</a>
												</p>
											</c:if> <c:if test="${item.status eq '8' || item.status eq '18' }">
												<p class="btn_m1">
													<a href="#" onclick="return_popup('${item.cart_no}');">반품신청</a>
												</p>
											</c:if> <c:if test="${item.status eq '8' }">
												<p class="btn_m1">
													<a href="#" onclick="exchange_popup('${item.cart_no}');">교환신청</a>
												</p>
											</c:if> <c:if test="${item.status eq '12' }">
												<p class="btn_m1">
													<a href="#" onclick="refunds_popup('${item.cart_no}');">환불신청</a>
												</p>
											</c:if></td>
									</tr>
								</c:forEach>
