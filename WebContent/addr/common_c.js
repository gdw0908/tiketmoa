...
function Check_AlphaNumericSpecial(checkStr, value) {
	...
	if (result==1)
	{
		for (var k = 0;  k < checkStr.value.length;  k++) {
        ...
        for(var i = 0; i < length; i++) {
		 	...
		 	if(siNm[i].firstChild.nodeValue=="세종특별자치시"){
		 		if(liNm[i].childNodes.length == '0') {
			 		...
			 		jibunAddr = siNm[i].firstChild.nodeValue+' '+emdNm[i].firstChild.nodeValue;
			 		addrDetail = tempLnbr;
		 		}else{
			 		...
			 		jibunAddr = siNm[i].firstChild.nodeValue+' '+emdNm[i].firstChild.nodeValue+' '+liNm[i].firstChild.nodeValue;
			 		addrDetail = tempLnbr;
		 		}
		 		...
		 		if(liNm[i].childNodes.length == '0') {
			 		...
			 		jibunAddr = siNm[i].firstChild.nodeValue+' '+sggNm[i].firstChild.nodeValue+' '+emdNm[i].firstChild.nodeValue;
			 		addrDetail = tempLnbr;
			 	}else{
			 		...
			 		jibunAddr = siNm[i].firstChild.nodeValue+' '+sggNm[i].firstChild.nodeValue+' '+emdNm[i].firstChild.nodeValue+' '+liNm[i].firstChild.nodeValue;
			 		addrDetail = tempLnbr;
			 	}
		 	}
			...
			if(i%2=='0'){
				html += "<ul class='item'>";
				html += "<li class='road'><a href='javascript:setAddr(\""+ newaddr+"\",\""+addrDetail+"\",\""+zip+"\",\""+jibunAddr+"\")'>"+zipCl[i].firstChild.nodeValue+' '+jibun+"</li>";
				html += "</ul>";
			}
		 	else{
		 		html += "<ul class='item'>";
	 	...
	 	if(length>0)
	 		html+=getPaging(intCountPerPage[0].firstChild.nodeValue,'5',totalCount[0].firstChild.nodeValue,document.check.currentPage.value,"");
...
function updateAreaList() {
  ...
  for(var i = 0; i < length; i++) {
		if(isExceptArea(names[i].firstChild.nodeValue)){
			...
			option = new Option(names[i].firstChild.nodeValue, values[i].firstChild.nodeValue);
...
function setParentValue(newaddr, jibun, zipcode){
	try{
		window.opener.setAddr(newaddr, jibun, zipcode);	
	}catch(e){
		alert("부모창에 setAddr([도로명주소], [지번주소], [우편번호]) 함수를 구현하세요");
	}
