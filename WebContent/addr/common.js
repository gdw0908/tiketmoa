  ...
  if(xmlHttp.status == 200){
       ...
       var totalCount		= xmlHttp.responseXML.getElementsByTagName('totalCount');
        ...
        for(var i = 0; i < length; i++) {
			...
			if(i%2=='0'){
				html += "<ul class='item'>";
				html += "<li class='road'><a href='#' onclick='return setParentValue(\""+ newaddr+"\",\""+jibun+"\",\""+zip+"\")'>"+"[도로명]"+newaddr+"</a></li>";
				html += "<li class='road'>"+"[지 번]"+jibun+"</li>";
				html += "<li class='road'>"+"[우편번호]"+zipCl[i].firstChild.nodeValue+"</li>";
				html += "</ul>";
			}
		 	else{
		 		html += "<ul class='item'>";
		 		html += "<li class='road2'><a href='#' onclick='return setParentValue(\""+ newaddr+"\",\""+jibun+"\",\""+zip+"\")'>"+"[도로명]"+newaddr+"</a></li>";
		 		html += "<li class='road2'>"+"[지번]"+jibun+"</li>";
		 		html += "<li class='road2'>"+"[우편번호]"+zipCl[i].firstChild.nodeValue+"</li>";
		 		html += "</ul>";
		 	}
 	   }
	 	if(length>0)
