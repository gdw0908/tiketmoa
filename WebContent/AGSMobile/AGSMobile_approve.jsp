...
<%@page language="java" import="kr.co.allthegate.mobile.AGSMobile"%>
...
$(function(){
	AGSMoblile_Response_data.submit();
});
</script>
</head>
<%
...
%>
<body>
<form name = "AGSMoblile_Response_data" id = "AGSMoblile_Response_data" method = "post" action = "/mobile/mypage/shopping/cart/index.do?mode=m_pay_result">
	<input type = "hidden" name = "userid" value = "<%=info.get("UserId")%>">			<!-- 회원아이디 -->
	<input type = "hidden" name = "orderno" value = "<%=data.get("OrdNo")%>">			<!-- 주문번호 -->
	<input type = "hidden" name = "dealno" value = "<%=data.get("DealNo")%>">			<!-- 거래번호-->
	<input type = "hidden" name = "admno" value = "<%=data.get("AdmNo")%>">				<!-- 승인번호-->
	<input type = "hidden" name = "partialmm" value = "<%=data.get("PartialMm")%>">		<!-- 할부개월수 -->
	<input type = "hidden" name = "admtime" value = "<%=data.get("AdmTime")%>">			<!-- 승인시각 -->
	<input type = "hidden" name = "useremail" value = "<%=info.get("UserEmail")%>">		<!-- 주문자 이메일 -->
	<input type = "hidden" name = "amt" value = "<%=data.get("Amt")%>">					<!-- 결제금액 -->
	
	<input type = "hidden" name = "paytype" value = "<%=ret.get("paytype")%>">			<!-- 결제방법(카드,무통장 ,,,) -->
	<input type = "hidden" name = "cardselect" value = "<%=info.get("CardSelect")%>">	<!-- 결제사 -->
	
	<input type = "hidden" name = "status" value = "<%=ret.get("status")%>">			<!-- 결제성공여부 -->
	<input type = "hidden" name = "storeid" value = "<%=data.get("StoreId")%>">			<!-- 결제성공여부 -->
	
	<input type = "hidden" name = "transaction" value = "<%=transaction%>">
	<input type = "hidden" name = "tracking_id" value = "<%=tracking_id%>">
	<input type = "hidden" name = "ordernm" value = "<%=info.get("OrdNm")%>">
	<input type = "hidden" name = "subty" value = "<%=info.get("SubTy")%>">
	
	
	<!-- 가상계좌  -->
	<input type = "hidden" name = "NetCancelId" value = "<%=data.get("NetCancelId")%>">
	<input type = "hidden" name = "EscrowYn" value = "<%=data.get("EscrowYn")%>">
	<input type = "hidden" name = "EscrowSendNo" value = "<%=data.get("EscrowSendNo")%>">
	<input type = "hidden" name = "SuccessTime" value = "<%=data.get("SuccessTime")%>">
	<input type = "hidden" name = "VirtualNo" value = "<%=data.get("VirtualNo")%>">
	<input type = "hidden" name = "BankCode" value = "<%=data.get("BankCode")%>">
	<input type = "hidden" name = "DueDate" value = "<%=data.get("DueDate")%>">
